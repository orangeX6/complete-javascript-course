...
const ages = [
  calcAgeXY(years[0]),
  calcAgeXY(years[3]),
  calcAgeXY(years[years.length - 1]),
];
...
if (john.calcBMI() > mark.calcBMI()) {
  console.log(
    `${john.firstName} ${john.lastName}'s(${john.bmi})BMI is higher than ${mark.firstName} ${mark.lastName}'s'(${mark.bmi})`
  );
} else {
  console.log(
    `${mark.firstName} ${mark.lastName}'s (${mark.bmi})BMI is higher than ${john.firstName} ${john.lastName}'s(${john.bmi}) BMI`
  );
}
